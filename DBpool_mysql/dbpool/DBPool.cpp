...
{  
    if(instance_ == NULL)  
    {  
        instance_ = new DBPool("tcp://127.0.0.1:3306", "root", "123456", 100);  
    }  
    return instance_;  
} 

DBPool::DBPool(std::string url_, std::string user_, std::string password_, int maxSize_) 
{  
    this->user = user_;  
