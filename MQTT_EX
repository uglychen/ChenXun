...
public:
    ...
    void init(std::string address, std::string clientid)
    {
        p_client = new  mqtt::async_client(address, clientid);
        callback cb;
        p_client->set_callback(cb);

        mqtt::itoken_ptr conntok = p_client->connect();
    ...
    void sendMessage(const std::string message)
    {
       // mqtt::async_client client(ADDRESS, CLIENTID);
        ...
        pubmsg->set_qos(QOS);
        p_client->publish(TOPIC, pubmsg)->wait_for_completion(TIMEOUT);
        std::cout << "OK" << std::endl;
    }
