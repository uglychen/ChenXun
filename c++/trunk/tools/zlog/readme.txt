该文档为zlog使用手册，详见：https://hardysimpson.github.io/zlog/UsersGuide-CN.html

快速使用步骤：

1  编译和安装zlog

下载zlog-latest-stable.tar.gz  ，与本文件同级目录下已有压缩包 zlog.tar.gz
...
配置文件是test_default.conf，和test_hello.conf一模一样，最后执行程序的输出也一样。区别在于这里用了dzlog API，内含一个默认的zlog_category_t。详见下面手册内容。
