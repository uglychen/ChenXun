...
bool activemq_trade_consumer::init(const std::string& url, const std::string& queue)
{
    try 
	{
		std::auto_ptr<cms::ConnectionFactory> ptr_connectionFactory(cms::ConnectionFactory::createCMSConnectionFactory(url));
		...
		_ptr_recv_queue = std::auto_ptr<cms::Queue>(_ptr_session->createQueue(queue));
