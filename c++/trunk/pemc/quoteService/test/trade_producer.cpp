...
bool activemq_trade_producer::init(const std::string& broker_url)
{
	try
	{
		std::unique_ptr<cms::ConnectionFactory> ptr_connectionFactory(cms::ConnectionFactory::createCMSConnectionFactory(broker_url));
...
void activemq_trade_producer::send_message(const Destination* destination, const std::string& msg, const std::string& id)
{
	try
	{
		std::unique_ptr<cms::MessageProducer> ptr_send_message = std::unique_ptr<cms::MessageProducer>(_ptr_session->createProducer(destination));
		ptr_send_message->setDeliveryMode(DeliveryMode::NON_PERSISTENT);
		//发送消息
		std::unique_ptr<cms::TextMessage> ptr_message(_ptr_session->createTextMessage(msg));
...
void activemq_trade_producer::send_message(const std::string& queueName, const std::string& msg)
{
	try
	{
		std::unique_ptr<cms::Destination> ptr_destination = std::unique_ptr<cms::Destination>(_ptr_session->createQueue(queueName));
		std::unique_ptr<cms::MessageProducer> ptr_send_message = std::unique_ptr<cms::MessageProducer>(_ptr_session->createProducer(ptr_destination.get()));
		ptr_send_message->setDeliveryMode(DeliveryMode::NON_PERSISTENT);
		//发送消息
		std::unique_ptr<cms::TextMessage> ptr_message(_ptr_session->createTextMessage(msg));
		ptr_send_message->send(ptr_message.get());
